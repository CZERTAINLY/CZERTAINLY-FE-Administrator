...
export default function CustomAttributeWidget({resource, resourceUuid, attributes}: Props) {
    ...
    const resourceCustomAttributesContents = useSelector(customAttributesSelectors.resourceCustomAttributesContents(resource, resourceUuid));
    ...
    const addCustomAttribute = (attributeUuid: string, content: BaseAttributeContentModel[] | undefined) => {
        if (content) {
            dispatch(customAttributesActions.updateCustomAttributeContent({
                resource: resource,
    ...
    const removeCustomAttribute = (attributeUuid: string) => {
        dispatch(customAttributesActions.removeCustomAttributeContent({
            resource: resource,
            resourceUuid: resourceUuid,
            attributeUuid: attributeUuid,
        }));
    ...
    const options = useMemo(() => resourceCustomAttributes.filter(r => !loadedAttributes?.find(a => a.uuid === r.uuid)).map(r => ({
        label: r.properties.label,
        value: r,
    })), [resourceCustomAttributes, loadedAttributes]);

    return (<Widget title={"Custom Attributes"} busy={isFetchingResourceCustomAttributes || isUpdatingContent}>
        <AttributeViewer attributes={loadedAttributes} descriptors={resourceCustomAttributes} viewerType={ATTRIBUTE_VIEWER_TYPE.ATTRIBUTE_EDIT}
                         onSubmit={addCustomAttribute} onRemove={removeCustomAttribute}/>
        {options && options.length > 0 && <Form onSubmit={() => {
        }}>
            {({form}) => (
                <BootstrapForm>
                    <h6><b>Add custom attribute</b></h6>
                    <Row>
                        <Col xs="6" sm="6" md="6" lg="6" xl="6">
                            <Field key={"selectCustomAttribute"}
                                ...
                                {({input}) =>
                                    (<Select
                                        ...
                                        placeholder={`Add...`}
                                        isClearable={true}
                                        onChange={(v) => {input.onChange(v); setAttribute(v?.value ?? undefined)}}
                                    />)}
                            </Field>
                        </Col>
                        <Col xs="6" sm="6" md="6" lg="6" xl="6">
                            {attribute && <ContentValueField descriptor={attribute} onSubmit={(uuid, content) => {
