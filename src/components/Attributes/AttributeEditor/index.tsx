...
interface Props {
    ...
    groupAttributesCallbackAttributesGlobalModal?: AttributeDescriptorModel[];
    setGroupAttributesCallbackAttributes?: Function;
    setGroupAttributesCallbackAttributesGlobalModal?: Function;
...
export default function AttributeEditor({
    ...
    groupAttributesCallbackAttributesGlobalModal = emptyGroupAttributesCallbackAttributes,
    setGroupAttributesCallbackAttributes = () => emptyGroupAttributesCallbackAttributes,
    setGroupAttributesCallbackAttributesGlobalModal = () => emptyGroupAttributesCallbackAttributes,
    usesGlobalModal = undefined,
}: Props) {
    ...
    const [prevDescriptorsGlobalModal, setPrevDescriptorsGlobalModal] = useState<AttributeDescriptorModel[]>();

    // used to check if attributes have changed
    const [prevAttributes, setPrevAttributes] = useState<AttributeResponseModel[]>();
    const [prevAttributesGlobalModal, setPrevAttributesGlobalModal] = useState<AttributeResponseModel[]>();

    const [prevGroupDescriptors, setPrevGroupDescriptors] = useState<AttributeDescriptorModel[]>();
    const [prevGroupDescriptorsGlobalModal, setPrevGroupDescriptorsGlobalModal] = useState<AttributeDescriptorModel[]>();

    // options for selects
    const [options, setOptions] = useState<{ [attributeName: string]: { label: string; value: any }[] }>({});
    const [optionsGlobalModal, setOptionsGlobalModal] = useState<{ [attributeName: string]: { label: string; value: any }[] }>({});
    ...
    const [previousFormValuesGlobalModal, setPreviousFormValuesGlobalModal] = useState<{ [name: string]: any }>({});
    ...
    const [previousCallbackDataGlobalModal, setPreviousCallbackDataGlobalModal] = useState<{ [callbackId: string]: any }>({});
    ...
    const groupedAttributesDescriptorsGlobalModal: { [key: string]: (DataAttributeModel | InfoAttributeModel | CustomAttributeModel)[] } =
        useMemo(() => {
            const grouped: { [key: string]: (DataAttributeModel | InfoAttributeModel | CustomAttributeModel)[] } = {};
            [...attributeDescriptors, ...groupAttributesCallbackAttributesGlobalModal].forEach((descriptor) => {
                if (isDataAttributeModel(descriptor) || isInfoAttributeModel(descriptor) || isCustomAttributeModel(descriptor)) {
                    const groupName = descriptor.properties.group || "__";
                    grouped[groupName] ? grouped[groupName].push(descriptor) : (grouped[groupName] = [descriptor]);
                }
            ...
            return grouped;
        }, [attributeDescriptors, groupAttributesCallbackAttributesGlobalModal]);
    ...
    useEffect(() => {
        // run this effect only when attribute descriptors or attributes changes
        if (!usesGlobalModal) return;
        if (
            attributeDescriptors === prevDescriptorsGlobalModal &&
            attributes === prevAttributesGlobalModal &&
            groupAttributesCallbackAttributesGlobalModal === prevGroupDescriptorsGlobalModal
        )
            return;
        ...
        const descriptorsToLoad =
            attributeDescriptors === prevDescriptorsGlobalModal && attributes === prevAttributesGlobalModal
                ? groupAttributesCallbackAttributesGlobalModal ?? []
                : [...attributeDescriptors, ...groupAttributesCallbackAttributesGlobalModal];

        setPrevGroupDescriptorsGlobalModal(groupAttributesCallbackAttributesGlobalModal);
        setPrevDescriptorsGlobalModal(attributeDescriptors);
        setPrevAttributesGlobalModal(attributes);
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
        opts = { ...opts, ...newOptions };
        setOptionsGlobalModal({ ...optionsGlobalModal, ...opts });

        // now all fields are loaded in form, so set those formState.values as previousFormValues to prevent calling doCallback on form change
        setPreviousFormValuesGlobalModal(formState.values);
    }, [
        id,
        attributeDescriptors,
        groupAttributesCallbackAttributesGlobalModal,
        ...
        optionsGlobalModal,
        dispatch,
        prevDescriptorsGlobalModal,
        prevAttributesGlobalModal,
        prevGroupDescriptorsGlobalModal,
        buildCallbackMappings,
    ]);
    ...
    const doCallbacksGlobalModal = useCallback(() => {
        if (previousFormValuesGlobalModal === formState.values) return;
        ...
        for (const key in formState.values) {
            if (key.startsWith(`__attributes__${id}__`)) {
                for (const attrKey in formState.values[key]) {
                    if (
                        previousFormValuesGlobalModal[key] === undefined ||
                        previousFormValuesGlobalModal[key][attrKey] === undefined ||
                        previousFormValuesGlobalModal[key][attrKey] !== formState.values[key][attrKey]
                    ) {
                        changedAttributes[attrKey] = {
                            previous:
                                previousFormValuesGlobalModal[key] !== undefined &&
                                previousFormValuesGlobalModal[key][attrKey] !== undefined
                                    ? previousFormValuesGlobalModal[key][attrKey]
                                    : undefined,
                            current: formState.values[key][attrKey],
                        };
                    }
    ...
    useEffect(() => {
        ref.current(doCallbacksGlobalModal);
    }, [doCallbacksGlobalModal]);
    ...
    useEffect(() => {
        ...
        if (usesGlobalModal === true || usesGlobalModal === undefined) return;
    ...
    }, [callbackData, options, previousCallbackData, usesGlobalModal]);

    useEffect(() => {
        if (previousCallbackDataGlobalModal === callbackData) return;
        if (usesGlobalModal === false || usesGlobalModal === undefined) return;

        for (const callbackId in callbackData) {
            if (callbackData[callbackId] === previousCallbackDataGlobalModal[callbackId]) continue;
            if (!callbackData[callbackId]) continue;
            ...
            if (Array.isArray(callbackData[callbackId])) {
                ...
                setGroupAttributesCallbackAttributesGlobalModal(groupCallbackAttributes);
        ...
        setPreviousCallbackDataGlobalModal(callbackData);
    }, [callbackData, optionsGlobalModal, previousCallbackDataGlobalModal]);
   ...
   */
    ...
    const globalAttr = useMemo(() => {
        const attrs: JSX.Element[] = [];
        for (const group in groupedAttributesDescriptorsGlobalModal)
            attrs.push(
                <Widget key={group} title={group === "__" ? "" : group} busy={isRunningCb}>
                    {groupedAttributesDescriptorsGlobalModal[group].map((descriptor) => (
                        <div key={descriptor.name}>
                            <Attribute
                                busy={isRunningCb}
                                name={`__attributes__${id}__.${descriptor.name}`}
                                descriptor={descriptor}
                                options={optionsGlobalModal[`__attributes__${id}__.${descriptor.name}`]}
                            />
                        </div>
                    ))}
                </Widget>,
        ...
        return attrs;
    }, [groupedAttributesDescriptorsGlobalModal, isRunningCb, id, optionsGlobalModal]);

    return usesGlobalModal ? <>globalAttr{globalAttr}</> : <>{attrs}</>;
}
