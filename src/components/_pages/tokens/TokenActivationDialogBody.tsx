...
export default function TokenActivationDialogBody({
   tokenUuid,
   visible,
   onClose
}: Props) {
   ...
   const activationAttributes = useSelector(selectors.activationAttributes);
    ...
    const [activationGroupAttributesCallbackAttributes, setActivationGroupAttributesCallbackAttributes] = useState<AttributeDescriptorModel[]>([]);
   ...
   const isBusy = useMemo(
      () => isFetchingTokens || isFetchingActivationAttributes,
      [isFetchingTokens, isFetchingActivationAttributes]
   );
   ...
   const onActivateSubmit = useCallback(
      ...
      (values: any) => {
         ...
         const activationAttribs: AttributeRequestModel[] = activationAttributes && activationAttributes.length > 0
            ?
            collectFormAttributes("activationAttributes", [...(activationAttributes ?? []), ...activationGroupAttributesCallbackAttributes], values) || []
            :
            []
            ;

         dispatch(actions.activateToken({
             uuid: tokenUuid,
