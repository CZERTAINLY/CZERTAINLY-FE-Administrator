...
const operationValues = ['AUTH', 'CHANGE', 'CREATE', 'DELETE', 'DISABLE', 'ENABLE', 'ISSUE', 'REQUEST', 'RESET', 'REVOKE', 'START', 'STOP'];
const operationStatusValues = ['FAILURE', 'SUCCESS'];
const originationValues = ['ACCESS', 'ADMINISTRATOR', 'AUDIT_LOG', 'BE', 'CA', 'CLIENT', 'END_ENTITY', 'END_ENTITY_CERTIFICATE', 'END_ENTITY_PROFILE', 'FE', 'RA_PROFILE'];

export class AuditLogsMock implements model.AuditLogsApi {
  getObjects(): Observable<string[]> {
    return of(originationValues).pipe(
      delay(randomDelay()),
    );
  }
  getOperations(): Observable<string[]> {
    return of(operationValues).pipe(
      delay(randomDelay()),
    );
  }
  getStatuses(): Observable<string[]> {
    return of(operationStatusValues).pipe(
      delay(randomDelay()),
    );
  }
