...
const listTokenProfiles: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.listTokenProfiles.match
      ),
      switchMap(
...
const getTokenProfileDetail: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.getTokenProfileDetail.match
      ),
      switchMap(

         action => deps.apiClients.tokenProfiles.getTokenProfile({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid }).pipe(

            map(
               profileDto => slice.actions.getTokenProfileDetailSuccess({
            ...
            catchError(
               err => of(
                  ...
                  appRedirectActions.fetchError({ error: err, message: "Failed to get Token Profile detail" })
               )
            )
...
const createTokenProfile: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.createTokenProfile.match
      ),

      switchMap(

         action => deps.apiClients.tokenProfiles.createTokenProfile({ tokenInstanceUuid: action.payload.tokenInstanceUuid, addTokenProfileRequestDto: transformTokenProfileAddRequestModelToDto(action.payload.tokenProfileAddRequest) }
         ).pipe(

            mergeMap(
               obj => of(
                  slice.actions.createTokenProfileSuccess({ uuid: obj.uuid, tokenInstanceUuid: action.payload.tokenInstanceUuid }),
...
const updateTokenProfile: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.updateTokenProfile.match
      ),
      switchMap(

         action => deps.apiClients.tokenProfiles.editTokenProfile({ uuid: action.payload.profileUuid, tokenInstanceUuid: action.payload.tokenInstanceUuid, editTokenProfileRequestDto: transformTokenProfileEditRequestModelToDto(action.payload.tokenProfileEditRequest) }
         ).pipe(
            ...
            catchError(
               err => of(slice.actions.updateTokenProfileFailure({ error: extractError(err, "Failed to update profile") }))
            )
...
const enableTokenProfile: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.enableTokenProfile.match
      ),

      switchMap(

         action => deps.apiClients.tokenProfiles.enableTokenProfile({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid }).pipe(

            map(
               () => slice.actions.enableTokenProfileSuccess({ uuid: action.payload.uuid })
            ),
...
const disableTokenProfile: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.disableTokenProfile.match
      ),

      switchMap(

         action => deps.apiClients.tokenProfiles.disableTokenProfile({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid }).pipe(

            map(
               () => slice.actions.disableTokenProfileSuccess({ uuid: action.payload.uuid })
            ),
...
const deleteTokenProfile: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.deleteTokenProfile.match
      ),
      switchMap(

         action => deps.apiClients.tokenProfiles.deleteTokenProfile({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid }).pipe(
...
const bulkEnableProfiles: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.bulkEnableTokenProfiles.match
      ),
      switchMap(

         action => deps.apiClients.tokenProfiles.enableTokenProfiles({ requestBody: action.payload.uuids }).pipe(

            map(
               () => slice.actions.bulkEnableTokenProfilesSuccess({ uuids: action.payload.uuids })
            ),
...
const bulkDisableProfiles: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.bulkDisableTokenProfiles.match
      ),

      switchMap(

         action => deps.apiClients.tokenProfiles.disableTokenProfiles({ requestBody: action.payload.uuids }).pipe(

            map(
               () => slice.actions.bulkDisableTokenProfilesSuccess({ uuids: action.payload.uuids })
            ),
...
const bulkDeleteProfiles: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.bulkDeleteTokenProfiles.match
      ),
      switchMap(
...
const updateKeyUsage: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      switchMap(
         ...
         action => deps.apiClients.tokenProfiles.updateKeyUsages({ tokenInstanceUuid: action.payload.tokenInstanceUuid, tokenProfileUuid: action.payload.uuid, tokenProfileKeyUsageRequestDto: action.payload.usage }).pipe(

            map(
               () => slice.actions.updateKeyUsageSuccess({ uuid: action.payload.uuid,  usage: action.payload.usage.usage })
            ),
...
const bulkUpdateKeyUsage: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      switchMap(
         ...
         action => deps.apiClients.tokenProfiles.updateKeysUsages({ bulkTokenProfileKeyUsageRequestDto: action.payload.usage }).pipe(
