...
import { AttributeRequestModel, AttributeResponseModel } from 'types/attributes';
    ...
    return action$.pipe(
        ...
        switchMap((action) => {
            if (action.payload.resource === Resource.Vaults) {
                ...
                if (!currentVault || currentVault.uuid !== action.payload.resourceUuid || !currentAttributes || !currentCustomAttributes) {
                    ...
                    return of(
                        slice.actions.updateCustomAttributeContentFailure({
                            ...
                            error: extractError(error, 'Failed to update custom attribute content'),
                        }),
                    );
                }

                const descriptor = state.customAttributes.resourceCustomAttributes.find(
                    (entry) => entry.uuid === action.payload.attributeUuid,
                );
                const existingAttribute = currentCustomAttributes.find((entry) => entry.uuid === action.payload.attributeUuid);

                if (!existingAttribute && !descriptor) {
                ...
                const updatedAttribute: AttributeResponseModel = existingAttribute
                    ? { ...existingAttribute, content: action.payload.content }
                    : {
                          uuid: action.payload.attributeUuid,
                        ...
                        mergeMap((vault) =>
                            of(
                                slice.actions.updateCustomAttributeContentSuccess({
                                    resource: action.payload.resource,
                                    resourceUuid: action.payload.resourceUuid,
                                    customAttributes: (vault.customAttributes ?? []).map(transformAttributeResponseDtoToModel),
                                }),
            ...
            if (action.payload.resource === Resource.VaultProfiles) {
                ...
                const existingAttribute = currentAttributes.find((entry) => entry.uuid === action.payload.attributeUuid);
                ...
                if (!currentVault || currentVault.uuid !== action.payload.resourceUuid || !currentAttributes || !currentCustomAttributes) {
                    ...
                    return of(
                        slice.actions.removeCustomAttributeContentFailure({
                            resource: action.payload.resource,
                            resourceUuid: action.payload.resourceUuid,
                            error: extractError(error, 'Failed to remove custom attribute content'),
                        }),
                    );
                }
