...
const listCryptographicKeys: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.listCryptographicKeys.match
      ),
      switchMap(
...
const getCryptographicKeyDetail: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.getCryptographicKeyDetail.match
      ),
      switchMap(

         action => deps.apiClients.cryptographicKeys.getKey({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid }).pipe(

            map(
               profileDto => slice.actions.getCryptographicKeyDetailSuccess({
...
const getAttributesDescriptors: AppEpic = (action$, state, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.listAttributeDescriptors.match
      ),
      switchMap(

         action => deps.apiClients.cryptographicKeys.listCreateKeyAttributes({ 
            ...
            map(
               descriptors => slice.actions.listAttributeDescriptorsSuccess({
            ...
            catchError(
               err => of(
                  slice.actions.listAttributeDescriptorsFailure({ error: extractError(err, "Failed to get Attribute to create key") }),
...
const createCryptographicKey: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.createCryptographicKey.match
      ),

      switchMap(

         action => deps.apiClients.cryptographicKeys.createKey({ tokenInstanceUuid: action.payload.tokenInstanceUuid,
            tokenProfileUuid: action.payload.tokenProfileUuid,
            ...
            mergeMap(
               obj => of(
                  slice.actions.createCryptographicKeySuccess({ uuid: obj.uuid, tokenInstanceUuid: action.payload.tokenInstanceUuid }),
...
const updateCryptographicKey: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.updateCryptographicKey.match
      ),
      switchMap(

         action => deps.apiClients.cryptographicKeys.editKey({ uuid: action.payload.profileUuid, tokenInstanceUuid: action.payload.tokenInstanceUuid, editKeyRequestDto: transformCryptographicKeyEditRequestModelToDto(action.payload.cryptographicKeyEditRequest) }
         ).pipe(

            mergeMap(

               cryptographicKeyDto => iif(
            ...
            catchError(
               err => of(slice.actions.updateCryptographicKeyFailure({ error: extractError(err, "Failed to update Key") }))
            )
...
const enableCryptographicKey: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.enableCryptographicKey.match
      ),

      switchMap(

         action => deps.apiClients.cryptographicKeys.enableKey({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid, requestBody: action.payload.keyItemUuid }).pipe(
...
const disableCryptographicKey: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.disableCryptographicKey.match
      ),

      switchMap(

         action => deps.apiClients.cryptographicKeys.disableKey({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid,  requestBody: action.payload.keyItemUuid }).pipe(

            map(
               () => slice.actions.disableCryptographicKeySuccess({ uuid: action.payload.uuid, keyItemUuid: action.payload.keyItemUuid })
            ),
...
const deleteCryptographicKey: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.deleteCryptographicKey.match
      ),
      switchMap(

         action => deps.apiClients.cryptographicKeys.deleteKey({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid,  requestBody: action.payload.keyItemUuid }).pipe(
            ...
            mergeMap(
               ...
               () => iif(
                  ...
                  () => !!action.payload.redirect,
                  of(
                     slice.actions.deleteCryptographicKeySuccess({ uuid: action.payload.uuid, keyItemUuid: action.payload.keyItemUuid }),
                     appRedirectActions.redirect({ url: action.payload.redirect! })
                  ),
                  of(
                     slice.actions.deleteCryptographicKeySuccess({ uuid: action.payload.uuid, keyItemUuid: action.payload.keyItemUuid })
...
const bulkEnableCryptographicKeys: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.bulkEnableCryptographicKeys.match
      ),
      switchMap(

         action => deps.apiClients.cryptographicKeys.enableKeys({ requestBody: action.payload.uuids }).pipe(

            map(
               () => slice.actions.bulkEnableCryptographicKeysSuccess({ uuids: action.payload.uuids })
            ),
...
const bulkDisableCryptographicKeys: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.bulkDisableCryptographicKeys.match
      ),

      switchMap(

         action => deps.apiClients.cryptographicKeys.disableKeys({ requestBody: action.payload.uuids }).pipe(

            map(
               () => slice.actions.bulkDisableCryptographicKeysSuccess({ uuids: action.payload.uuids })
            ),
...
const bulkDeleteCryptographicKeys: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.bulkDeleteCryptographicKeys.match
      ),
      switchMap(
...
const updateKeyUsage: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.updateKeyUsage.match
      ),

      switchMap(

         action => deps.apiClients.cryptographicKeys.updateKeyUsages1({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid, updateKeyUsageRequestDto: action.payload.usage }).pipe(

            map(
               () => slice.actions.updateKeyUsageSuccess({ uuid: action.payload.uuid,  usage: action.payload.usage.usage, keyItemsUuid: action.payload.usage.uuids || [] })
            ),
...
const bulkUpdateKeyUsage: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.bulkUpdateKeyUsage.match
      ),
      switchMap(

         action => deps.apiClients.cryptographicKeys.updateKeysUsages1({ bulkKeyUsageRequestDto: action.payload.usage }).pipe(

            map(
               () => slice.actions.bulkUpdateKeyUsageSuccess({ })
            ),
...
const compromiseCryptographicKey: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.compromiseCryptographicKey.match
      ),
      ...
      switchMap(
         ...
         action => deps.apiClients.cryptographicKeys.compromiseKey({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid, compromiseKeyRequestDto: transformCryptographicKeyCompromiseModelToDto(action.payload.request) }).pipe(
            ...
            catchError(
               err => of(
                  slice.actions.compromiseCryptographicKeyFailure({ error: extractError(err, "Failed to mark the Key as Compromised") }),
...
const bulkCompromiseCryptographicKeys: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.bulkCompromiseCryptographicKeys.match
      ),
      switchMap(
...
const destroyCryptographicKey: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.destroyCryptographicKey.match
      ),

      switchMap(

         action => deps.apiClients.cryptographicKeys.destroyKey({ tokenInstanceUuid: action.payload.tokenInstanceUuid, uuid: action.payload.uuid, requestBody: action.payload.keyItemUuid }).pipe(

            map(
               () => slice.actions.destroyCryptographicKeySuccess({ uuid: action.payload.uuid, keyItemUuid: action.payload.keyItemUuid })
            ),
...
const bulkDestroyCryptographicKeys: AppEpic = (action$, state$, deps) => {
   ...
   return action$.pipe(
      ...
      filter(
         slice.actions.bulkDestroyCryptographicKeys.match
      ),
      switchMap(

         action => deps.apiClients.cryptographicKeys.destroyKeys({ requestBody: action.payload.uuids }).pipe(

            map(
               errors => slice.actions.bulkDestroyCryptographicKeys({ uuids: action.payload.uuids })
            ),
